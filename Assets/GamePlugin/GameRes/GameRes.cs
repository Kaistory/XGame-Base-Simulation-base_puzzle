...
namespace mygame.sdk
{
    ...
    public class GameRes
    {
        ...
        {
            timeStartGame = SdkUtil.CurrentTimeMilis();
            timeEndGame = 0;
            duarationPlayLv = 0;
        }
        ...
        public static void endLevel()
        {
            if (timeStartGame > 0)
            {
                timeEndGame = SdkUtil.CurrentTimeMilis();
            }
            else
            {
                timeEndGame = 0;
        ...
        public static bool AddRes(RES_type type, int value, string desUpdate, bool isLog = true, float delayTime = 0,
            Action<int, int> cbAddRes = null)
        {
            string nameRes = type.ToString().ToLower();
            var _currVa = getRes(nameRes);
            var result = false;
            //ItemDisplay.OnItemChange?.Invoke(_currVa, _currVa + value, type, delayTime, .75f);
            if (delayTime > 0)
            {
                UnityMainThreadDispatcher.Instance()
                    .Enqueue(WaiAddRes(nameRes, value, desUpdate, isLog, delayTime, cbAddRes));
                //return true;
                result = true;
            }
            else
            {
                result = doAddRes(nameRes, value, desUpdate, isLog);
            }

            return result;
        }

        public static bool AddRes(string nameRes, int value, string desUpdate, bool isLog = true, float delayTime = 0,
            Action<int, int> cbAddRes = null)
        {
            if (delayTime > 0)
            {
        ...
        public static bool AddRes(RES_type type, int value)
        {
            string nameRes = type.ToString().ToLower();
            int _currVa = getRes(nameRes);
            {
                return doAddRes(nameRes, value, "", false);
            }
        }

        private static IEnumerator WaiAddRes(string nameRes, int value, string desUpdate, bool isLog, float delayTime,
            Action<int, int> cbAddRes = null)
        {
        ...
        private static bool doAddRes(string nameRes, int value, string desUpdate, bool isLog,
            Action<int, int> cbAddRes = null)
        {
            int _currVa = getRes(nameRes);
            if (cbAddRes != null)
            ...
            if (isLog)
            {
                if (value > 0)
                {
                    ...
                    Myapi.LogEventApi.Instance().logResource(LevelCommon(), "source", nameRes, 1, nameRes, value,
                        _currVa, _currVa + value, desUpdate);
                }
                else
                {
                    Dictionary<string, string> dicParam = new Dictionary<string, string>();
                    ...
                    Myapi.LogEventApi.Instance().logResource(LevelCommon(), "sink", nameRes, 1, nameRes, value, _currVa,
                        _currVa + value, desUpdate);
                }
            }
