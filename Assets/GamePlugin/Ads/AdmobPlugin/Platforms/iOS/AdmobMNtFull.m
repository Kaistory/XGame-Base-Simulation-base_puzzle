...
int GGMNtCountAfterlic = -100000;
int GGMNtScrWidth = 40;
int GGMNtmarginBtClose = 40;

@interface AdmobMNtFull () <GADNativeAdLoaderDelegate, GADVideoControllerDelegate, GADNativeAdDelegate>
...
- (bool)show:(int)timeBtClose timeDelay:(int)timeDelay autoCloseWhenClick:(bool)isAudto
{
    ...
    if (self.isLoaded && self.nativeAd != nil) {
        ...
        GGMNtScrWidth = unityView.bounds.size.width;
        if (GGMNtScrWidth >= unityView.bounds.size.height) {
            GGMNtmarginBtClose = 40;
        } else {
            GGMNtmarginBtClose = 30;
        }
        ...
        if (self.nativeAdView == nil) {
            ...
            self.ntFullBtCloseView.frame = CGRectMake(GGMNtScrWidth - GGMNtmarginBtClose - 10, 40, 20.0, 20.0);
            ...
            self.ntFullBtClose.frame = CGRectMake(GGMNtScrWidth - GGMNtmarginBtClose - 10, 40, 20.0, 20.0);
        ...
        self.countShow = [[NSUserDefaults standardUserDefaults] integerForKey:@"ggmntfull_count_show"];
        self.countShow++;
        [[NSUserDefaults standardUserDefaults] setInteger:self.countShow forKey:@"ggmntfull_count_show"];
        if (GGMNtCountAfterlic < -100) {
            GGMNtCountAfterlic = (int)[[NSUserDefaults standardUserDefaults] integerForKey:@"ggmntfull_count_aflick"];
        }
        GGMNtCountAfterlic++;
        [[NSUserDefaults standardUserDefaults] setInteger:GGMNtCountAfterlic forKey:@"ggmntfull_count_aflick"];
        return true;
    } else {
...
- (void) cbCounTime:(int)flagNham
{
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 1 * NSEC_PER_SEC), dispatch_get_main_queue(), ^{
        self.countTimeClose--;
        NSInteger coclick = [[NSUserDefaults standardUserDefaults] integerForKey:@"ggmntfull_count_click"];
        if (self.countTimeClose > 0) {
            ...
            if (flagNham == -1) {
                if (GGMNtCountAfterlic == 1) {
                    per = self.adParent.ntFullFirstRan;
                } else if (GGMNtCountAfterlic > 1) {
                    per = self.adParent.ntFullOtherRan + self.adParent.ntFullIncrease * (GGMNtCountAfterlic - 1);
                }
            ...
            if (isNhamlic && self.countTimeClose >= -1 && ((coclick * 100) <= (self.countShow * self.adParent.ntFullPer))) {
                ...
                if (flagNham == 1) {
                    ...
                    self.ntFullBtClose.frame = CGRectMake(GGMNtScrWidth - GGMNtmarginBtClose - btSize / 2, 50 - btSize / 2 , btSize, btSize);
                } else {
                    self.ntFullBtClose.frame = CGRectMake(GGMNtScrWidth - GGMNtmarginBtClose - 20, 30, 40.0, 40.0);
                }
                NSLog(@"mysdk: ads full nt %@ admobnt flagNham clik=%ld, show=%ld cf=%d fran=%d sran=%d non=%d inc=%d after=%d bt=%f", self.placement, coclick, self.countShow, self.adParent.ntFullPer, self.adParent.ntFullFirstRan, self.adParent.ntFullOtherRan, self.adParent.ntFullCountNonAfter, self.adParent.ntFullIncrease, GGMNtCountAfterlic, btSize);
            }
        }
    });
}
...
- (void)adLoader:(GADAdLoader *)adLoader didReceiveNativeAd:(GADNativeAd *)nativeAd {
    ...
    if (self.isLoading) {
        ...
        __weak AdmobMNtFull *weakSelf = self;
...
- (void)nativeAdDidRecordClick:(GADNativeAd *)nativeAd {
    ...
    if (self.isAllowWclick) {
        ...
        NSInteger coclick = [[NSUserDefaults standardUserDefaults] integerForKey:@"ggmntfull_count_click"];
        coclick++;
        [[NSUserDefaults standardUserDefaults] setInteger:coclick forKey:@"ggmntfull_count_click"];
        GGMNtCountAfterlic = -self.adParent.ntFullCountNonAfter;
        [[NSUserDefaults standardUserDefaults] setInteger:GGMNtCountAfterlic forKey:@"ggmntfull_count_aflick"];
    }
